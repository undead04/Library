...
namespace Library.Server.SubjectReponsitory
{
    public class SubjectReponsitory : ISubjectReponsitory
    {
        ...
        {
            var subjects=await context.subjects.Where(su=>su.subjectClassRooms.Any(su=>su.ClassRoomId==ClassId)).ToListAsync();
            return subjects.Select(su => new SubjectDTO
            {
                Id = su.Id,
                Name = su.Name,
